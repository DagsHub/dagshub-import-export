...
def reimport_repo(source_url: str, destination_url: str, git=True, dvc=True, repo_bucket=True):
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        if dvc:
            ...
            copy_rclone_dvc(source_repo.repoApi, destination_repo.repoApi)

        if repo_bucket:
            logger.info("Copying repository bucket data")
            copy_rclone_repo_bucket(source_repo.repoApi, destination_repo.repoApi)
...
def main() -> None:
    ...
    reimport_repo(source_url, destination_url, git=False, dvc=False, repo_bucket=True)
