...
def _export_versions(mlflow_client, model_dct, versions, output_dir, opts):
    ...
    for j, vr in enumerate(versions):
        if (
            ...
            and (len(opts.stages) > 0 and vr.current_stage.lower() not in opts.stages)
        ):
            continue
        if len(opts.versions) > 0 and vr.version not in opts.versions:
            continue
        _export_version(
            mlflow_client,
            vr,
...
def _export_version(mlflow_client, vr, output_dir, aliases, output_versions, failed_versions, j, num_versions, opts):
    ...
    _logger.info(f"Exporting model version {j + 1}/{num_versions}: {msg} to '{_output_dir}'")
