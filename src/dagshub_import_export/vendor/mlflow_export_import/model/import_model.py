...
from dagshub_import_export.vendor.mlflow_export_import.client.client_utils import (
...
class BaseModelImporter:
    ...
    def __init__(self, mlflow_client=None, import_permissions=False, import_source_tags=False, await_creation_for=None):
        """
...
class ModelImporter(BaseModelImporter):
    ...
    def import_model(self, model_name, input_dir, experiment_name, delete_model=False, verbose=False):
        """
        :param model_name: Model name.
        :param input_dir: Input directory.
...
class BulkModelImporter(BaseModelImporter):
    ...
    def import_model(self, model_name, input_dir, delete_model=False, verbose=False):
        ...
        for vr in model_dct["versions"]:
            ...
            if not dst_run_info:
                ...
                except (RestException, MlflowExportImportException) as e:
                    msg = {
                        "model": model_name,
    ...
    def import_version(self, model_name, src_vr, dst_run_id):
        ...
        model_source: str = src_vr["source"]
        if model_source.startswith("models:/"):
            _logger.info(
                f"Not importing model {src_vr['name']}({src_vr['version']} because its source is a registered model: {model_source} and not an artifact URI"
            )
            return None
...
def _extract_model_path(source, run_id):
    ...
    if idx == -1:
        raise MlflowExportImportException(
            f"Cannot find run ID '{run_id}' in registered model version source field '{source}'", http_status_code=404
        )
    model_path = source[1 + idx + len(run_id) :]
    pattern = "artifacts"

    idx = model_path.find(pattern)
    if idx == -1:  # Bizarre - sometimes there is no 'artifacts' after run_id
        model_path = ""
    else:
        model_path = model_path[1 + idx + len(pattern) :]
    return model_path
...
@opt_import_source_tags
@click.option(
    "--await-creation-for", help="Await creation for specified seconds.", type=int, default=None, show_default=True
)
@opt_verbose
def main(
    input_dir, model, experiment_name, delete_model, import_permissions, import_source_tags, await_creation_for, verbose
):
