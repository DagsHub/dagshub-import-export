...
class HttpClient(BaseHttpClient):
    ...
    """
    def __init__(self, api_name, username, password, host=None):
        """
    ...
    def _get(self, resource, params=None):
        ...
        rsp = requests.get(uri, headers=self._mk_headers(), params=params, timeout=_TIMEOUT, auth=self.auth)
    ...
    def _delete(self, resource):
        uri = self._mk_uri(resource)
        rsp = requests.delete(uri, headers=self._mk_headers(), timeout=_TIMEOUT, auth=self.auth)
    ...
    def _mutator(self, method, resource, data=None):
        uri = self._mk_uri(resource)
        rsp = method(uri, headers=self._mk_headers(), data=data, timeout=_TIMEOUT, auth=self.auth)
        return self._check_response(rsp)
...
class DatabricksHttpClient(HttpClient):
    ...
    def __init__(self, username, password, host=None):
        super().__init__("api/2.0", host=host, username=username, password=password)
...
class MlflowHttpClient(HttpClient):
    ...
    """
    def __init__(self, username, password, host=None):
        super().__init__("api/2.0/mlflow",  host=host, username=username, password=password)
